...
class RnnEncoder(nn.Module):
    def __init__(self, hidden_size, num_layers, embed_model):        
        ...
        self.rnn = nn.RNN(embed_model.embedding_dim, hidden_size, num_layers, batch_first=True)
...
class RnnDecoder(nn.Module):
    ...
    def forward(self, hidden, max_length):
        ...
        for _ in range(max_length):
            ...
            output, hidden = self.rnn(embedded, hidden) # output: (batch_size, 1, hiddenn_size); hidden: tuple of (h, c) shape (num_layers, batch_size, hidden_size)
